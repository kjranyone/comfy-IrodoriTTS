...
def _free_vram(amount: int, device: torch.device) -> None:
    """Ask ComfyUI's model manager to release VRAM before we place a model."""
    if device.type == "cpu" or amount <= 0:
        return
    try:
        import comfy.model_management as model_management

        model_management.free_memory(int(amount), device)
    except Exception as exc:
        print(f"[IrodoriTTS] ComfyUI memory free request failed: {exc}", flush=True)


def _node_data_dir() -> Path:
    """Per-node cache directory for downloaded tokenizers and codecs."""
...
class SamplingRequest:
    ...
    lora_adapters: tuple[tuple[str, float], ...] = ()
...
class InferenceRuntime:
    def __init__(
        ...
        *,
        ...
        self.watermarker = SilentCipherWatermarker(
            enabled=bool(key.silentcipher_watermark_enabled),
    ...
    def from_key(cls, key: RuntimeKey) -> InferenceRuntime:
        ...
        _free_vram(_module_bytes(model), model_device)
        ...
        tokenizer_cache_dir = _node_data_dir() / "tokenizers"
        ...
        tokenizer = PretrainedTextTokenizer.from_pretrained(
            ...
            cache_dir=None if text_tokenizer_is_local else str(tokenizer_cache_dir),
        )
        if (
        ...
        if model_cfg.use_caption_condition:
            ...
            caption_tokenizer = PretrainedTextTokenizer.from_pretrained(
                ...
                cache_dir=None if caption_tokenizer_is_local else str(tokenizer_cache_dir),
            )
            if (
                not model_cfg.use_pretrained_text_encoder
        ...
        codec_cache_dir = _node_data_dir() / "codecs" / codec_repo_safe
        codec_cache_dir.mkdir(parents=True, exist_ok=True)
        _free_vram(1536 * 1024 * 1024, codec_device)
    ...
    def _prepare_lora_for_request_inner(
        ...
        *,
        ...
        for adapter_path, scale in adapters:
            resolved_path = self._resolve_lora_adapter_path(adapter_path)
            if resolved_path is not None:
                resolved.append((resolved_path, float(scale)))
        if not resolved:
        ...
        for resolved_path, _scale in resolved:
        ...
        self.model.eval()
        if not self._lora_base_scaling:
            self._lora_base_scaling = {
                (module_name, str(adapter_name)): float(value)
                for module_name, module in self.model.named_modules()
                if hasattr(module, "scaling")
                for adapter_name, value in getattr(module, "scaling", {}).items()
            }
        ...
        if base_model is not None and hasattr(base_model, "set_adapter"):
            ...
            if hasattr(self.model, "active_adapter"):
                self.model.active_adapter = list(adapter_names)
        else:
        ...
        for module_name, module in self.model.named_modules():
            if not hasattr(module, "scaling"):
                continue
            for adapter_name, user_scale in zip(
                adapter_names,
                [scale for _path, scale in resolved],
                strict=True,
            ):
                key = (module_name, adapter_name)
                if key not in self._lora_base_scaling:
                    continue
                module.scaling[adapter_name] = self._lora_base_scaling[key] * user_scale

        @contextmanager
        def _restore_lora_scaling() -> Iterator[None]:
            try:
                yield
            finally:
                for module_name, module in self.model.named_modules():
                    ...
                    for adapter_name in list(getattr(module, "scaling", {})):
                        key = (module_name, str(adapter_name))
                        if key in self._lora_base_scaling:
                            module.scaling[adapter_name] = self._lora_base_scaling[key]

        return _restore_lora_scaling()
        ...
        *,
        ...
        _log(
            (
                ...
                "codec_watermark={} silentcipher_watermark={} mode={} seconds={} steps={} seed={} candidates={} decode_mode={}"
            ).format(
        ...
        with (
            ...
            if self.key.silentcipher_watermark_enabled:
                if self.watermarker.ready:
                    ...
                    msg = "warning: SilentCipher watermarking was requested but is unavailable; audio was left unwatermarked."
                    messages.append(msg)
                    _log(msg)
            else:
                _log("info: SilentCipher watermarking is disabled for this run.")
    ...
    def ensure_loaded(self) -> None:
        """Restore model and codec to their configured devices after offload()."""
        if not self._offloaded:
            return
        _free_vram(_module_bytes(self.model), self.model_device)
        self.model.to(self.model_device)
        _free_vram(_module_bytes(self.codec.model), self.codec_device)
    ...
    def offload(self) -> None:
        with self._infer_lock:
            ...
            self.codec.device = torch.device("cpu")
    ...
    def unload(self) -> None:
        with self._infer_lock:
            for attr in ("model", "tokenizer", "caption_tokenizer", "codec", "watermarker"):
