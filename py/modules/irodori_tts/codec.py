...
from ..hf_assets import fetch_file
...
class DACVAECodec:
    ...
    def load(
        ...
        # Resolve location: local_dir cache > hf:// prefix > weights.pth > repo_id passthrough
        location = str(repo_id).strip()
        if location.startswith("hf://"):
            location = location[len("hf://") :]

        if local_dir is not None:
            # Keep each codec repo in its own directory so weights never mix.
            cache = Path(local_dir)
            cache.mkdir(parents=True, exist_ok=True)

            cached = sorted(cache.glob("*.pth"))
            if cached:
                location = str(cached[0])
            else:
                try:
                    repo_files = [f for f in list_repo_files(repo_id) if f.endswith(".pth")]
                except Exception:
                    repo_files = []
                filename = repo_files[0] if repo_files else "weights.pth"
                try:
                    location = str(fetch_file(repo_id, filename, cache))
                    print(f"[codec] dacvae: hf://{repo_id}/{filename} -> {location}", flush=True)
                except Exception:
                    # Let DACVAE.load surface a clear error for invalid paths/repos.
                    location = repo_id
