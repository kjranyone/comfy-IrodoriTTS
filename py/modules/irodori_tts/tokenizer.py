...
from ..hf_assets import fetch_tokenizer_dir
...
class PretrainedTextTokenizer:
    ...
    def from_pretrained(
        ...
        if cache_dir is not None and not local_files_only:
            try:
                source = str(fetch_tokenizer_dir(repo_id, cache_dir))
                resolved_local_files_only = True
            except Exception as exc:
                print(
                    f"[tokenizer] self-managed download failed ({exc}); "
                    "falling back to the Hugging Face cache",
                    flush=True,
                )
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            ...
            revision=revision if source == repo_id else None,
        )
